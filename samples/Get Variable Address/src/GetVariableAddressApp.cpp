...
class GetVariableAddressApp : public AppNative {
public:
	void setup();
    void update();
	void draw();
...
void GetVariableAddressApp::setup()
{
...
void GetVariableAddressApp::setupScriptEngine()
{
    // get a reference to our global engine.
    asIScriptEngine* engine = as::Script::getEngine();
    
    // register a print function.
    int r = engine->RegisterGlobalFunction("void print( const string &in )", asMETHOD(GetVariableAddressApp,print), asCALL_THISCALL_ASGLOBAL, this );  assert( r >= 0 );
...
void GetVariableAddressApp::update()
{
    if( mScript ){
...
void GetVariableAddressApp::draw()
{
    if( mScript ){
...
//! console wrapper
void GetVariableAddressApp::print( const std::string &s )
{
    console() << s << endl;
}

CINDER_APP_NATIVE( GetVariableAddressApp, RendererGl )
